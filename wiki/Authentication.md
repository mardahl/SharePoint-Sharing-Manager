...
- Registers an Entra app with **application** permissions `Sites.FullControl.All` (SharePoint), `User.ReadWrite.All` (SharePoint), `Sites.FullControl.All` (Graph), and `User.Read.All` (Graph).
- SharePoint `User.ReadWrite.All` is requested so OneDrive pre-provisioning (`P`, see [[OneDrive-Pre-Provisioning]]) can call the User Profile Service via `Request-PnPPersonalSite`. Existing app-only registrations need it added manually (API permissions > SharePoint > Application > `User.ReadWrite.All` > grant admin consent); the tool never PATCHes permissions onto an existing registration.
