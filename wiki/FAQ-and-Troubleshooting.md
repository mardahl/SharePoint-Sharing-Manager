...
The OneDrive secondary-admin feature needs Graph `User.Read.All` (app-only) or the delegated mode's default consent set. If this app registration predates that scope, add it manually - see [[Authentication]]. This never falls back to a partial-match user lookup; it fails closed. Scans and revokes are unaffected.
